...
/*富文本编辑图片上传配置*/
const uploadConfig = {
//  action:  'http://upload.qiniu.com/',  // 必填参数 图片上传地址
    ...
    image: function image() {
        ...
        if (fileInput === null) {
            ...
            fileInput.addEventListener('change', function () {
            	...
            	if(size > uploadConfig.size){
            		alert('上传图片不能超过1M')
            	}else{
            		// 创建formData
	                var formData = new FormData();
	                
	                axios.get('/api/yw/qiniu/token?bucket=yuwei')
			   		.then((res)=>{
			   			if(res.data.success){
			   				...
			   				.then((res)=>{
			   					let imgSrc = 'http://file.kxdz2.com/' + res.data.key;
			   					let length = self.quill.getSelection(true).index;
			   					self.quill.insertEmbed(length, 'image', imgSrc);
			   					self.quill.setSelection(length + 1);
			   					
			   					fileInput.value = ''
			   					console.log(res,imgSrc,length)
