<template>
	...
	<div class="case-width">
		...
		<div class="search">
			<span>课程名称：</span>
        	<Input v-model.trim="search.name" placeholder="课程名称" clearable style="width: 200px" />
		    ...
		    <Button type="primary" @click="showMask">添加课程</Button>
		</div>
		...
		<!--添加课程弹窗-->
		<Modal title="添加课程数据" v-model="modal9" :loading="loading" fullscreen ok-text="提交" @on-ok='subData'>
			<div class="modal-width">
		        <div class="mask">
		        	<span>课程名称：</span>
		        	<Input v-model.trim="modal.name" autofocus placeholder="添加课程名称" clearable style="width: 900px" />
		        </div>
		        
		        <div class="mask">
		        	<span>课程简介：</span>
		        	<Input v-model.trim="modal.digest" type="textarea" clearable :autosize="{minRows: 2,maxRows: 5}" placeholder="添加课程简介" style="width: 900px" />
		        </div>
		...
		<!--修改课程弹窗-->
		<Modal title="修改课程数据" v-model="modal88" :loading="loading" fullscreen ok-text="提交" @on-ok='modifyData'>
			<div class="modal-width">
		        <div class="mask">
		        	<span>课程名称：</span>
		        	<Input v-model.trim="modal.name" autofocus placeholder="添加课程名称" clearable style="width: 900px" />
...
//			   				_this.clearModal();
		   ...
		   modify(id){
		   		...
		   		//根据ID获取要修改的数据
		   		let _this = this;
		   		let user = '/api/yw/course/findById/' + id;
...
//			   				_this.clearModal();
		   ...
		   online(id){
		   		...
		   		this.$Modal.confirm({
                    ...
                    onOk: () => {
                        let user = '/api/yw/course/release/' + id;
				   		_this.$axios.put(user)
				   		.then((res)=>{
				   			if(res.data.success){
		   ...
		   remove(id){
		   		...
		   		this.$Modal.confirm({
                    title: '是否确定该条数据下线？',
                    onOk: () => {
                        let user = '/api/yw/course/offLine/' + id;
				   		_this.$axios.put(user)
				   		.then((res)=>{
				   			if(res.data.success){
		   ...
		   getChildClass(){
		   		let _this = this;
		   		let user = '/api/yw/category/list/14/child';
		   		_this.$axios.get(user)
		   		.then((res)=>{
		   			if(res.data.success){
