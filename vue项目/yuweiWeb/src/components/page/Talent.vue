<template>
	...
	<div class="case-width">
		...
		<div class="search">
			<span>姓名：</span>
        	<Input v-model.trim="search.name" placeholder="达人姓名" clearable style="width: 200px" />
		    ...
		    <Button type="primary" @click="showMask">添加达人</Button>
		</div>
		...
		<!--添加达人弹窗-->
		<Modal title="添加达人数据" v-model="modal9" :loading="loading" fullscreen ok-text="提交" @on-ok='subData'>
			<div class="modal-width">
		        <div class="mask">
		        	<span>姓名：</span>
		        	<Input v-model.trim="modal.name" autofocus placeholder="添加达人姓名" clearable style="width: 900px" />
		        </div>
		        
		        <div class="mask">
		        	<span>手机号：</span>
		        	<Input v-model.trim="modal.phone" autofocus placeholder="添加达人手机号" clearable style="width: 900px" />
		        </div>
		        
		        <div class="mask">
		        	<span>昵称：</span>
		        	<Input v-model.trim="modal.nikeName" autofocus placeholder="添加达人昵称" clearable style="width: 900px" />
		        </div>
		        
		        <div class="mask">
		        	<span>达人简介：</span>
		        	<Input v-model.trim="modal.digest" type="textarea" clearable :autosize="{minRows: 2,maxRows: 5}" placeholder="添加达人简介" style="width: 900px" />
		        </div>
		...
		<!--修改课程弹窗-->
		<Modal title="修改达人数据" v-model="modal88" :loading="loading" fullscreen ok-text="提交" @on-ok='modifyData'>
			<div class="modal-width">
		        <div class="mask">
		        	<span>姓名：</span>
		        	<Input v-model.trim="modal.name" autofocus placeholder="添加达人姓名" clearable style="width: 900px" />
...
//			   				_this.clearModal();
		   ...
		   modify(id){
		   		...
		   		//根据ID获取要修改的数据
		   		let _this = this;
		   		let user = '/api/yw/talent/findById/' + id;
...
//			   				_this.clearModal();
		   ...
		   online(id){
		   		...
		   		this.$Modal.confirm({
                    title: '是否禁用该条数据？',
                    onOk: () => {
                        let user = '/api/yw/talent/isEnable/' + id;
				   		_this.$axios.put(user)
				   		.then((res)=>{
				   			if(res.data.success){
		   ...
		   getChildClass(){
		   		let _this = this;
		   		let user = '/api/yw/category/list/18/child';
		   		_this.$axios.get(user)
		   		.then((res)=>{
		   			if(res.data.success){
