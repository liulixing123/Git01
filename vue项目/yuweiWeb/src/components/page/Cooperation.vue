<template>
	...
	<div class="case-width">
		...
		<div class="search">
			<span>名称：</span>
        	<Input v-model.trim="search.name" placeholder="机构名称" clearable style="width: 200px" />
		    ...
		    <Button type="primary" @click="showMask">添加机构</Button>
		</div>
		...
		<!--添加课程弹窗-->
		<Modal title="添加机构数据" v-model="modal9" :loading="loading" fullscreen ok-text="提交" @on-ok='subData'>
			<div class="modal-width">
		        <div class="mask">
		        	<span>名称：</span>
		        	<Input v-model.trim="modal.name" autofocus placeholder="添加机构名称" clearable style="width: 900px" />
		        </div>
		        
		        <div class="mask">
		        	<span>联系电话：</span>
		        	<Input v-model.trim="modal.phone" autofocus placeholder="添加机构联系电话" clearable style="width: 900px" />
		        </div>
		        
		        <div class="mask">
		        	<span>机构简介：</span>
		        	<Input v-model.trim="modal.digest" type="textarea" clearable :autosize="{minRows: 2,maxRows: 5}" placeholder="添加机构简介" style="width: 900px" />
		        </div>
		...
		<!--修改课程弹窗-->
		<Modal title="修改机构数据" v-model="modal88" :loading="loading" fullscreen ok-text="提交" @on-ok='modifyData'>
			<div class="modal-width">
		        <div class="mask">
		        	<span>名称：</span>
		        	<Input v-model.trim="modal.name" autofocus placeholder="添加机构名称" clearable style="width: 900px" />
		        ...
		        <div class="mask">
		        	<span>联系电话：</span>
		        	<Input v-model.trim="modal.phone" autofocus placeholder="添加机构手机号" clearable style="width: 900px" />
		        </div>
...
<script>
	...
	export default {
		...
		methods:{
		   ...
		   getData(){
		   	let _this = this;
		   	let user = '/api/yw/cooperation/list';
		   	this.$axios.get(user,{
		   		params:{
		   ...
		   subData(){
		   		...
		   		let user = '/api/yw/cooperation/save';
		   		if(_this.modal.name !== ''){
...
//			   				_this.clearModal();
		   ...
		   modify(id){
		   		...
		   		//根据ID获取要修改的数据
		   		let _this = this;
		   		let user = '/api/yw/cooperation/findById/' + id;
		   ...
		   modifyData(){
		   		...
		   		if(modal.name ==''){
		   			...
		   			let user = '/api/yw/cooperation/edit';
...
//			   				_this.clearModal();
		   ...
		   online(id){
		   		...
		   		this.$Modal.confirm({
                    ...
                    onOk: () => {
                        let user = '/api/yw/cooperation/isEnable/' + id;
				   		_this.$axios.put(user)
				   		.then((res)=>{
				   			if(res.data.success){
