<template>
	...
	<div class="case-width">
		...
		<div class="search">
			<span>姓名：</span>
        	<Input v-model.trim="search.name" placeholder="输入名字" clearable style="width: 150px" />
        	
        	<span>联系电话：</span>
        	<Input v-model.trim="search.phone" placeholder="输入联系电话" clearable style="width: 150px" />
        	
        	<span>开始时间：</span>
        	<DatePicker type="datetime" @on-chang='time1' format='yyyy-MM-dd HH:mm:ss' placeholder="选择开始时间" style="width: 150px"></DatePicker>
        	
        	<span>结束时间：</span>
        	<DatePicker type="datetime" @on-chang='time2' format='yyyy-MM-dd HH:mm:ss' placeholder="选择结束时间" style="width: 150px"></DatePicker>
        	
		    <Button type="primary" @click="searchData" style="margin-right: 20px;">查询</Button> 
		    <Button type="primary" @click="showMask">添加留言</Button>
		</div>
		...
		<!--添加课程弹窗-->
		<Modal title="添加留言数据" v-model="modal9" :loading="loading" fullscreen ok-text="提交" @on-ok='subData'>
			<div class="modal-width">
		        
		        <div class="mask">
		        	<span>姓名：</span>
		        	<Input v-model.trim="modal.name" placeholder="添加姓名" clearable style="width: 900px" />
		        </div>
		        
		        <div class="mask">
		        	<span>联系电话：</span>
		        	<Input v-model.trim="modal.phone" placeholder="添加联系电话" clearable style="width: 900px" />
		        </div>
		        ...
		        <div class="mask">
		        	<span>年龄：</span>
		        	<Input v-model.trim="modal.age" placeholder="添加年龄" clearable style="width: 900px" />
		        </div>
		        
		        <div class="mask">
		        	<span>回访时间：</span>
		        	<Select v-model="modal.visitTime" style="width:900px">
			...
			return {
				...
				columns7: [
                    ...
                    
                    {
                        ...
                        key: 'visitTime',
                        render: (h,params) =>{
                        	if(params.row.visitTime == '1'){
                        		return h('div',{
                        			domProps: {
										innerHTML: '上班'
									},
                        		})
                        	}else if(params.row.visitTime == '2'){
                        		return h('div',{
                        			domProps: {
										innerHTML: '下班'
									},
		   ...
		   subData(){
		   		...
		   		if(_this.modal.name !== ''){
		   			_this.$axios.post(user,{
						...
						age: _this.modal.age,
						visitTime: _this.modal.visitTime,
...
//			   				_this.clearModal();
		   ...
		   modify(id){
		   		...
		   		//根据ID获取要修改的数据
		   		let _this = this;
		   		let user = '/api/yw/message/findById/' + id;
		   		...
		   		.then((res)=>{
		   			...
		   			if(res.data.success){
					   	...
					   	modal.course = data.course;
					   	modal.age = data.age;
					   	modal.visitTime = data.visitTime;
...
//			   				_this.clearModal();
		   ...
		   online(id){
		   		...
		   		this.$Modal.confirm({
                    ...
                    onOk: () => {
                        let user = '/api/yw/message/isEnable/' + id;
				   		_this.$axios.put(user)
				   		.then((res)=>{
				   			if(res.data.success){
		   ...
		   getChildClass(){
		   		let _this = this;
		   		let user = '/api/yw/category/list/15/child';
		   		_this.$axios.get(user)
		   		.then((res)=>{
		   			if(res.data.success){
