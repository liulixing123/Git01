...
<script>
	...
	export default {
		...
		methods: {
			...
			getData(id) {
				let vm = this;
				let user = "http://118.31.21.214:9001/api/mgr/message-board/" + id;
			...
			getBrand() {
				...
				let user = 'http://118.31.21.214:9001/api/mgr/brand/list';
				axios.get(user, {
						params: {
							pageSize:7,
							currentPage:1,
			...
			getChannel() {
				...
				let user = 'http://118.31.21.214:9001/api/mgr/channel/list';
				axios.get(user, {
						params: {
							'isEnable':1,
						},
			...
			modifyData(){
				...
				let user = "http://118.31.21.214:9001/api/mgr/message-board/promoter-edit";
				axios.put(user,{
					'id':vm.mbId,
					'promoterId':level,
					'name':vm.data.name,
			...
			removeData(){
				let vm = this;
				let toke = storage.get('toke');
				let user = 'http://118.31.21.214:9001/api/mgr/message-board/enable/' + vm.mbId;
				axios.put(user,{},{
					headers: {
							'token': toke
						}
