	...
	<div class="message">
		<div class="fixed-top">
			<div class="message-title clearfix">
				...
				<div class="title-right right" @click="dataListOff()">
					<img src="../../static/img/brand_01.png" alt="" />
				</div>
			</div>
			<div class="title-bg">
			...
			<div class="message-width">
				<ul class="data-ul" :class="{'ul-margin':title}">
					<li v-for="(msg,index) in messageList" :key='index'>
						...
						<div class="data-btn" :class="{'btn-none':dis}" @click="routerLink(msg.id)">
							...
							<div class="forediv" @click.stop><button class="btn" @click="addMask(msg.id,true)">分配</button></div>
		...
		<div class="message-footer" v-show="can">
			<div class="title-bg-width">
				<div class="footer-l left" @click="cancel()">
					取消
				</div>
				<div class="footer-r right" @click="con()">
					批量分配
				</div>
			</div>
		...
		<setmask ref="setmask" :proData="propsOne"></setmask>
	</div>
</template>
	...
	export default {
		...
		methods: {
			...
			subData(page, size) {
				...
				let user = '/api/mgr/message-board/keFuList';
			...
			//点击分配传递数据
			addMask(data,bol){
				this.propsOne.id = data;
				this.propsOne.show = bol;
				this.$refs.setmask.addMask(this.propsOne);
			}
		},
		mounted() {
			this.subData(this.currentPage, this.pageSize);
